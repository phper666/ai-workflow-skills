...
> 初始化后的模式切换（含评审机制/合并模式），按 `docs/团队AI研发工作流-配置说明.md` 的「模式切换交互」三分流（了解 → 讲解；目标明确 → 确认后改；目标模糊 → 列选项）。
...
**接入完成提示（收尾必说）**：告知用户——
「接入完成。现在你可以说：'给 XX 模块建立 XX 需求的共识文档' 开始第一个需求；
想跟着教程跑，说'带我跑一遍快速上手'（见 docs/快速上手.md）。」
