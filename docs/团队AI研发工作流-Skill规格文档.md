...
| 6 | `tech-design` | 实现角色 | 8 后, 10-设计核验 | `docs/design/` | "出个技术方案" / "怎么实现" |
| 7 | `lesson-deposit` | 实现角色 | 10 后, 12 | `docs/lessons/` | "这个坑记一下" / "沉淀" |

**覆盖矩阵**（12 步全部有家，无孤儿）：

```
1 建共识 ──→ 2 发基线 ──→ 3 扫描 ──→ 4 PM闭环 ──→ 5 评审 ──→ 6 拆解
   [2]         [2]        [3]         [3]         [2]        [2+4]
7 契约 ──→ 8 复核 ──→ 9 澄清 ──→ 10 开发核验 ──→ 11 传播 ──→ 12 沉淀
  [4]      [4]→[6]      [4]       [6+4 双核验]    [5]      [7+4+3+5]
```
...
*规格版本：v1.1 · 日期：2026-08-15 · 决策记录：4+1 边界确认 · 步骤 10 归入契约核验 · Q-items 多平台适配 · 扫描双模式（结果一致）· 新增 tech-design/lesson-deposit 环节 · 交付核验双核验 · 变更摘要升级（版本分组+取代链）*
