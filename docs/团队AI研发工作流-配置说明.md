...
## 模式切换交互（合并模式 / 评审机制 / worktree 模式通用）

用户提到切换/修改某个模式时，AI 按用户意图分流，不一刀切弹选项：

1. **了解型**（问「X 模式是什么」「有哪些模式」「auto 和 always 区别」）
   → 只讲解（引用本文档对应模式表格），**不弹选项、不改配置**

2. **行动型 + 目标明确**（「切换成 always」「改成 manual」）
   → 复述目标（「确认将 worktree 模式改为 always？」）→ 用户确认 → 改团队配置.md 对应行 → 回读确认生效

3. **行动型 + 目标模糊**（「帮我切换 worktree 模式」，没说切哪个）
   → 列该配置项的全部模式选项（含一句话差异）→ 用户选 → 确认后改 → 回读确认

判定标准：用户话语里有没有「改成/切换成 X」的行动意图 + 有没有明确目标。
只是了解 → 讲解即止，不弹框不打扰。
